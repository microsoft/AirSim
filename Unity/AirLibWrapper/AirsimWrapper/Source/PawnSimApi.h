...
public:
  struct Params {
    ...
    Params(UnityPawn *pawn_val, const NedTransform *global_transform_val,
...
private:
  ...
  void setStartPosition(const AirSimVector &position,
                        const AirSimQuaternion &rotator);
...
public:
  ...
  virtual std::vector<ImageCaptureBase::ImageResponse>
  getImages(const std::vector<ImageCaptureBase::ImageRequest> &request)
      const override;
  virtual std::vector<uint8_t>
  getImage(const std::string &camera_name,
           ImageCaptureBase::ImageType image_type) const override;
  ...
  virtual msr::airlib::CameraInfo
  getCameraInfo(const std::string &camera_name) const override;
  ...
  virtual void setCameraFoV(const std::string &camera_name,
                            float fov_degrees) override;
  virtual void setDistortionParam(const std::string &camera_name,
                                  const std::string &param_name,
                                  float value) override;
  virtual std::vector<float>
  getDistortionParams(const std::string &camera_name) override;
  ...
  virtual const msr::airlib::Kinematics::State *
  getGroundTruthKinematics() const override;
  virtual const msr::airlib::Environment *
  getGroundTruthEnvironment() const override;
