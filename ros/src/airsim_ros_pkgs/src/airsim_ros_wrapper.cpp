...
sensor_msgs::PointCloud2 AirsimROSWrapper::get_lidar_msg_from_airsim(const msr::airlib::LidarData& lidar_data, const std::string& sensor_name, const std::string& vehicle_name) const
{
        ...
        for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
            if (!vehicle_name_ptr_pair.second->lidar_pubs.empty()) {
                for (auto& lidar_publisher : vehicle_name_ptr_pair.second->lidar_pubs) {
                    ...
                    sensor_msgs::PointCloud2 lidar_msg = get_lidar_msg_from_airsim(lidar_data, lidar_publisher.sensor_name, vehicle_name_ptr_pair.first);
