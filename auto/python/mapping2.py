...
class mapping():
	...
	def imageCallback(self, data):
		...
		output = drawTracks(orig_pts.astype(int), new_pts.astype(int, self.img_curr, (0, 255, 0))
		...
		test = PoseStamped()
		test.header.stamp = data.header.stamp
		test.header.frame_id = self.pose.header.frame_id
		test.pose.position.x = P1[0,3]
		test.pose.position.y = P1[1,3]
		test.pose.position.z = P1[2,3]

		orient = np.hstack((P1[0:3,0:3],[[0],[0],[0]]))
		print(orient)
		orient = np.vstack((orient,[0,0,0,1]))
		orient = quaternion_from_matrix(orient)

		test.pose.orientation.x = orient[0]
		test.pose.orientation.y = orient[1]
		test.pose.orientation.z = orient[2]
		test.pose.orientation.w = orient[3]

		self.test_pose.publish(test)
