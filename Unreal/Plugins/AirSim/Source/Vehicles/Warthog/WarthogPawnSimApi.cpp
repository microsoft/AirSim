...
void WarthogPawnSimApi::initialize()
{
    ...
    joystick_controls_ = msr::airlib::WarthogApiBase::WarthogControls();
}

std::string WarthogPawnSimApi::getRecordFileLine(bool is_header_line) const
{
    ...
    const auto& state = pawn_api_->getWarthogState();

    std::ostringstream ss;
    ss << common_line;
    ss << current_controls_.linear_vel << "\t" << current_controls_.angular_vel << "\t";
    ss << state.linear_vel << "\t" << state.angular_vel << "\t";

    return ss.str();
}

//these are called on render ticks
void WarthogPawnSimApi::updateRenderedState(float dt)
{
...
void WarthogPawnSimApi::updateRendering(float dt)
{
    PawnSimApi::updateRendering(dt);

    updateWarthogControls();
...
void WarthogPawnSimApi::updateWarthogControls()
{
    ...
    if (!vehicle_api_->isApiControlEnabled()) {
        ...
        vehicle_api_->setWarthogControls(current_controls_);
        pawn_api_->updateMovement(current_controls_);
    }
    else {
        UAirBlueprintLib::LogMessageString("Control Mode: ", "API", LogDebugLevel::Informational);
        current_controls_ = vehicle_api_->getWarthogControls();
        pawn_api_->updateMovement(current_controls_);
    }
    UAirBlueprintLib::LogMessageString("Linear Vel: ", std::to_string(current_controls_.linear_vel), LogDebugLevel::Informational);
    UAirBlueprintLib::LogMessageString("Angular Vel: ", std::to_string(current_controls_.angular_vel), LogDebugLevel::Informational);
}

//*** Start: UpdatableState implementation ***//
void WarthogPawnSimApi::resetImplementation()
{
...
void WarthogPawnSimApi::reportState(StateReporter& reporter)
{
