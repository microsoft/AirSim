...
public:
  ...
  void CopyCameraSettingsToSceneCapture(UCameraComponent *Src,
  ...
  getProjectionMatrix(const APIPCamera::ImageType image_type) const;

  USceneCaptureComponent2D *getCaptureComponent(const ImageType type,
                                                bool if_active);
...
private: // members
  ...
  // below is needed because TMap doesn't work with UPROPERTY, but we do have
  // -ve index
...
private: // methods
  ...
  void setNoiseMaterial(int image_type, UObject *outer,
                        FPostProcessSettings &obj,
                        const NoiseSetting &settings);
  void setDistortionMaterial(int image_type, UObject *outer,
                             FPostProcessSettings &obj);
  static void updateCameraPostProcessingSetting(FPostProcessSettings &obj,
                                                const CaptureSetting &setting);

  // CinemAirSim
  static void updateCameraSetting(UCineCameraComponent *camera,
                                  const CaptureSetting &setting,
                                  const NedTransform &ned_transform);
};
