	...
	std::vector<msr::airlib::ImageCaptureBase::ImageResponse>& responses, bool use_safe_method) const
{
	...
	//for (unsigned int i = 0; i < requests.size(); ++i) {
	APIPCamera* camera = cameras_->at(requests.at(0).camera_name);
	//TODO: may be we should have these methods non-const?
	visibilityChanged = const_cast<UnrealImageCapture*>(this)->updateCameraVisibility(camera, requests[0]) || visibilityChanged;
	//}
	...
	//{
	//APIPCamera* camera = cameras_->at(requests.at(0).camera_name);
	USceneCaptureComponent2D* capture = camera->getCaptureComponent(requests[0].image_type, false);
	...
	responses.push_back(ImageResponse());
	ImageResponse& response = responses.at(responses.size() - 1);
	...
	render_params.push_back(std::make_shared<RenderRequest::RenderParams>(capture, textureTarget, requests[0].pixels_as_float, requests[0].compress));
	...
	//  }
		...
		//    }
		//};
	RenderRequest render_request{ nullptr, nullptr }; //gameViewport, std::move(query_camera_pose_cb) };
