...
{
	std::string vehicleIdentifier = AFlyingPawn::getVehicleName();

	if (vehicleIdentifier.compare(std::string("Quad1")) == 0)
		joystick_.getJoyStickState(0, joystick_state_);
	if (vehicleIdentifier.compare(std::string("Quad2")) == 0)
		joystick_.getJoyStickState(1, joystick_state_);

    rc_data_.is_connected = true;

    if (rc_data_.is_connected) {
		rc_data_.throttle = joyStickToRC(0, joystick_state_.left_y);
		rc_data_.yaw = joyStickToRC(0, joystick_state_.left_x);
		rc_data_.roll = joyStickToRC(0, joystick_state_.right_x);
		rc_data_.pitch = joyStickToRC(0, joystick_state_.right_y);
...
float AFlyingPawn::joyStickToRC(unsigned int id, int16_t val)
{
    float valf = static_cast<float>(20000 - (10000*id));
