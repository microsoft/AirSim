...
msr::airlib::MavLinkHelper::HILConnectionInfo AFlyingPawn::getHILConnectionInfo()
{
    ...
    connection_info.vehicle_name = std::string(TCHAR_TO_UTF8(*HILVehicleName));
    connection_info.use_serial = HILUseSerial;
    connection_info.ip_address = std::string(TCHAR_TO_UTF8(*HILUdpIP));
    connection_info.ip_port = HILUdpPort;
    connection_info.serial_port = std::string(TCHAR_TO_UTF8(*HILSerialPortName));
    connection_info.baud_rate = HILSerialBaudrate;
