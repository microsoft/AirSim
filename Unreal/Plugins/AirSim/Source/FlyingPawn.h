...
public: //blueprint properties
    ...
    FString HILVehicleName = "Pixhawk";
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "HIL")
    bool HILUseSerial = true;    //false = UDP
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "HIL")
    FString HILSerialPortName = "*";    // '*' means find port automatically, or use correct COM port like 'COM6' on Windows or /dev/ttyACM0 for linux
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "HIL")
    int HILSerialBaudrate = 115200; 
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "HIL")
    FString HILUdpIP = "127.0.0.1"; 
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "HIL")
    int HILUdpPort = 14560;    // port that is publishing HIL_SENSOR messages, this needs to be routed to the actual drone.
