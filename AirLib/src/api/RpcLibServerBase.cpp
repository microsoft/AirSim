    ...
    : api_provider_(api_provider)
{
	...
	pimpl_->server.bind("simAddMarker", [&](double lat, double lon, float alt, float radius, float lifetime) -> void {
		GeoPoint lla(lat, lon, alt);
		return getVehicleSimApi("")->simAddMarker(lla, radius, lifetime);
	});
