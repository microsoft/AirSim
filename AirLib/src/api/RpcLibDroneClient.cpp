...
RpcLibDroneClient::RpcLibDroneClient(const string&  ip_address, uint16_t port, uint timeout_ms)
{
    pimpl_.reset(new impl(ip_address, port, timeout_ms));
}

RpcLibDroneClient::~RpcLibDroneClient()
{}

bool RpcLibDroneClient::ping()
{
    return pimpl_->client.call("ping").as<bool>();
}
RpcLibDroneClient::ConnectionState RpcLibDroneClient::getConnectionState()
{
...
bool RpcLibDroneClient::armDisarm(bool arm)
{
    return pimpl_->client.call("armDisarm", arm).as<bool>();
}
void RpcLibDroneClient::setOffboardMode(bool is_set)
{
    pimpl_->client.call("setOffboardMode", is_set);
}
void RpcLibDroneClient::setSimulationMode(bool is_set)
{
    pimpl_->client.call("setSimulationMode", is_set);
}
bool RpcLibDroneClient::takeoff(float max_wait_seconds)
{
    return pimpl_->client.call("takeoff", max_wait_seconds).as<bool>();
}
bool RpcLibDroneClient::land(float max_wait_seconds)
{
    return pimpl_->client.call("land", max_wait_seconds).as<bool>();
}
bool RpcLibDroneClient::goHome()
{
    return pimpl_->client.call("goHome").as<bool>();
}
void RpcLibDroneClient::start()
{
    pimpl_->client.call("start");
}
void RpcLibDroneClient::stop()
{
    pimpl_->client.call("stop");
}


bool RpcLibDroneClient::moveByAngle(float pitch, float roll, float z, float yaw, float duration)
{
    return pimpl_->client.call("moveByAngle", pitch, roll, z, yaw, duration).as<bool>();
}

bool RpcLibDroneClient::moveByVelocity(float vx, float vy, float vz, float duration, DrivetrainType drivetrain, const YawMode& yaw_mode)
{
    return pimpl_->client.call("moveByVelocity", vx, vy, vz, duration, drivetrain, RpcLibAdapators::YawMode(yaw_mode)).as<bool>();
}

bool RpcLibDroneClient::moveByVelocityZ(float vx, float vy, float z, float duration, DrivetrainType drivetrain, const YawMode& yaw_mode)
{
    return pimpl_->client.call("moveByVelocityZ", vx, vy, z, duration, drivetrain, RpcLibAdapators::YawMode(yaw_mode)).as<bool>();
}

bool RpcLibDroneClient::moveOnPath(const vector<Vector3r>& path, float velocity, float max_wait_seconds, DrivetrainType drivetrain, const YawMode& yaw_mode, float lookahead, float adaptive_lookahead)
{
...
bool RpcLibDroneClient::moveToPosition(float x, float y, float z, float velocity, float max_wait_seconds, DrivetrainType drivetrain, const YawMode& yaw_mode, float lookahead, float adaptive_lookahead)
{
...
bool RpcLibDroneClient::moveToZ(float z, float velocity, float max_wait_seconds, const YawMode& yaw_mode, float lookahead, float adaptive_lookahead)
{
...
bool RpcLibDroneClient::moveByManual(float vx_max, float vy_max, float z_min, float duration, DrivetrainType drivetrain, const YawMode& yaw_mode)
{
    return pimpl_->client.call("moveByManual", vx_max, vy_max, z_min, duration, drivetrain, RpcLibAdapators::YawMode(yaw_mode)).as<bool>();
}

bool RpcLibDroneClient::rotateToYaw(float yaw, float max_wait_seconds, float margin)
{
    return pimpl_->client.call("rotateToYaw", yaw, max_wait_seconds, margin).as<bool>();
}

bool RpcLibDroneClient::rotateByYawRate(float yaw_rate, float duration)
{
    return pimpl_->client.call("rotateByYawRate", yaw_rate, duration).as<bool>();
}

bool RpcLibDroneClient::hover()
{
    return pimpl_->client.call("hover").as<bool>();
}

bool RpcLibDroneClient::setSafety(SafetyEval::SafetyViolationType enable_reasons, float obs_clearance, SafetyEval::ObsAvoidanceStrategy obs_startegy,
...
//sim only
void RpcLibDroneClient::simSetPose(const Vector3r& position, const Quaternionr& orientation)
{
    pimpl_->client.call("simSetPose", RpcLibAdapators::Vector3r(position), RpcLibAdapators::Quaternionr(orientation));
}
vector<VehicleCameraBase::ImageResponse> RpcLibDroneClient::simGetImages(vector<DroneControllerBase::ImageRequest> request)
{
...
vector<uint8_t> RpcLibDroneClient::simGetImage(int camera_id, VehicleCameraBase::ImageType type)
{
...
void RpcLibDroneClient::confirmConnection()
{
    ...
    while (getConnectionState() != RpcLibDroneClient::ConnectionState::Connected)
    {
