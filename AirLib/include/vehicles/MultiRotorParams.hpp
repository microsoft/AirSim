                ...
                real_T rotor_z /* z relative to center of gravity */)
            {
                ...
                if (rotor_count == 4) {
                    ...
                    */
                    ...
                    // is counter clockwise, so the first vector (0,arm_lengths[0]) on the Y-axis, but when rotate -45 degrees ends up in the top right position
                    // where we want it.  The last vector (-x,0) is the negative x-axis, and when rotated -45 degrees ends up in the bottom right position.
