...
namespace simple_flight {
	...
	private:
		...
		const float k_phi = -16.75f; // roll angle //16.75
		const float k_theta = -26.75f; //  pitch angle //26.75
		const float k_psi = -1.0f; //  yaw angle //13
		const float k_roll = -450.0f; // roll rate //450
		const float k_pitch = -450.0f; // pitch rate //450
		const float k_yaw = -40000.0f; // yaw rate //400
		...
		GoalMode last_mode_;
		//float error[3] = { 0 };
		float ref_vec[10][3] = { 0 };
		float ref_sum[3] = { 0 };
		float velocity_integrator[3] = { 0 };
		...
		void model(float* y, float last_time_, float* y_out)
		{
			...
			float velocity_real[3] = { x[1][0],x[3][0],x[5][0] };
			float velocity_goal[3] = { reference[4],reference[3],reference[5] };
			...
			float lambda_theta_rate = 0.5f; 
			float lambda_phi = 0.95f; 
			float lambda_phi_rate = 0.95f; 
			float lambda_psi = 0.0004f;
