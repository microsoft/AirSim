...
public: //types
    ...
    // Separates base vehicle name from integer ordinal, when autogenerated vehicle count is > 1
    // Like so: SimpleFlight___0, SimpleFlight___1
	static constexpr char const * kGeneratedVehicleIdSeperator = "___";
    ...
    struct VehicleSetting {
		...
		// How many to auto-create
		// (Technically we could do away with 'auto_create' and just use this, but
		//  for now let's keep both, to maintain backward-compatibility)
		int count = 1;
		
		// Centimeters between vehicles as we place them on a regular grid
		float spacing_offset_cm = 500;
...
private:
        ...
        const std::string vehicle_name)
    {
		...
		vehicle_setting->count = settings_json.getInt("Count",
			vehicle_setting->count);
		vehicle_setting->spacing_offset_cm = settings_json.getFloat("SpacingOffsetCm",
			vehicle_setting->spacing_offset_cm);		
