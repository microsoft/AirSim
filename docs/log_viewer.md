...
You can open a log file, it supports .mavlink and PX4 *.ulg files.  AirSim can produce
mavlink log files if you set the following "PX4" vehicle parameter in your `settings.json` file:
```
{
    "SettingsVersion": 1.2,
    "SimMode": "Multirotor",
...
When you load a log you will see the "schema" found in that log file in a tree view on the left,
whatever metrics you select will be charted on the right side.  You can close each individual chart
with the little round close box in the top right of each chart and you can group charts so they
share the same vertical axis using the group charts button on the top toolbar.
