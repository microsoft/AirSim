...
* `simGetObjectPose`: Gets the pose of specified object in Unreal environment. Here the object means "actor" in Unreal terminology. They are searched by ActorLabel (as visible in World Outline) as well as name. The first match is returned. If no matches are found then NaN pose is returned.
